...
{
    int n = (finish_count() + 500) / 1000;

    /* Typically 7 or 8 milliseconds.
     * Less than or equal to 10 at any rate.
     */
    os_printf ( "TCP receive: %d in %d milliseconds:  ", len, n );
...
finish_count ( void )
{
    ...
    n = xthal_get_ccount () - start_count;
    //n /= 80;
    //os_printf ( "Reply in %d microseconds\n", n );
    return n/80;
}
