...
void
pin_input ( int gpio_no )
{
	gpio_output_set(0,0,0, 1<<gpio_no);
}

/* Read input value */
int
pin_input_read ( int gpio_no )
{
	// return (gpio_input_get()>>gpio_no) & 1;
	return gpio_input_get() & (1<<gpio_no);
}

/* Set output state high */
void
pin_output_high ( int gpio_no )
{
	int mask = 1 << gpio_no;
